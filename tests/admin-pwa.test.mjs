...
test("keeps the mobile chat composer visible, multiline, and easy to reset", () => {
    ...
    assert.match(adminPage, /id="chat-conversation"[^>]*><\/div>\s*<div class="chat-composer-wrap">/);
    assert.match(adminPage, /\.chat-composer-wrap\s*\{[\s\S]*?position:\s*relative;/);
    assert.match(adminPage, /\.app-shell\.is-chat-mode\s*\{[\s\S]*?position:\s*relative;[\s\S]*?height:\s*100dvh;/);
    assert.match(adminPage, /\.app-shell\.is-chat-mode:has\(#chat-composer-input:focus\) \.chat-composer-wrap/);
    assert.doesNotMatch(adminPage, /\.app-shell\.is-chat-mode\s*\{[\s\S]*?position:\s*fixed;/);
    assert.doesNotMatch(adminPage, /\.app-shell\.is-chat-mode\s*\{[\s\S]*?position:\s*absolute;/);
    assert.doesNotMatch(adminPage, /window\.visualViewport\?\.addEventListener/);
    assert.doesNotMatch(adminPage, /--chat-viewport-(?:height|top)/);
