...
- Chat bubbles use the shared white/plum/sage language with practical 44px action targets; assistant bubbles have a soft asymmetric lower-left corner and user bubbles mirror it. Preserve visible focus, live status and validation feedback, meaningful system / assistant / user states, and distinct ready, thinking, guided-fallback, and error status communication, plus reduced-motion-safe scrolling and bubble entry.
